...
\section{Case of diagonally separable initial state}\label{sec.separablestate}

We will now consider a class of states that I call diagonally separable states.
...
\begin{enumerate}  
    ...
    \text{ all } \left[\op{\rho}_n^A, \;\op{\rho}_{n^\prime}^A\right]&=0, &
...
Suppose \(\left[\op{\rho},\; \overline{\op{\rho}}_A\otimes\cdot\right] = 0\). Then there exists a basis \(\{\ket{\Psi_k\}\) of \(\hilb_A\otimes\hilb_B\) that diagonalizes both \(\op{\rho}\) and \(\overline{\op{\rho}}_A\otimes\cdot\). Write 
\begin{align*}
\op{\rho} & 
\end{align*}
\end{proof}
...
the special case where, at the time \(t_0\), the density operator takes the form
