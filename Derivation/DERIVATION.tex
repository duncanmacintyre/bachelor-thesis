...
We will find in Section \ref{sec.separablestate} that \(k=l=2\) for separable states. Therefore, if a separable state can transition into new states that initially had probability zero, the entropy will never decrease due to the perturbation. This conclusion is consistent with Ref. \cite{bracken} which proves that the Von Neumann entropy of separable states does not decrease.
...
\end{align*}
Because the trace of a matrix is invariant under a change of basis, we do not need to use the perturbation-diagonalized basis of \(\bket{n}{m}\). Indeed, we can use any choice of \(\bket{n}{m}\) such that
\[
