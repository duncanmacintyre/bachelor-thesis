...
Suppose we have a Hilbert space of the form \(\hilb = \hilb_A \otimes \hilb_B\). We can ask: what is the entropy of the subsystem \(\hilb_A\)? The calculations start out the same way as before, but now we are interested in the reduced density operator
\begin{equation}
\overline{\op{\rho}} = \sum_k \left(\cdot\otimes\bra{\alpha_k}\right) \op{\rho} \left(\cdot\otimes\ket{\alpha_k}\right)
\end{equation}
where \(\{\ket{\alpha_k}\}\) is some basis for \(\hilb_B\). We can consider this operator to act on the Hilbert space \(\hilb_A\).

As before, suppose that, at some fixed time \(t\), the density operator takes the form
\begin{equation}\label{eq.densdecompred}
\op{\rho}(t) = \sum_n \sigma_n \left(\ket{{\Psi_n}_A}\otimes\ket{{\Psi_n}_B}\right)\left(\bra{{\Psi_n}_A}\otimes\bra{{\Psi_n}_B}\right) 
\end{equation}
where \(\left\{\ket{{\Psi_n^{(0)}}_A}\otimes\ket{{\Psi_n^{(0)}}_B}\right\}\) is an orthonormal basis for \(\hilb\). Then
\[
\overline{\op{\rho}}(t) = \sum_n  \sigma_n \left(\sum_k|\braket{{\Psi_n}_B}{\alpha_k}|^2\right) \ket{{\Psi_n}_A}\bra{{\Psi_n}_A}.
\]
This sum may have several terms for each \(\ket{{\Psi_n}_A}\). We'd like to eliminate the duplicates. Let \(J\) be indices of unique \(\ket{{\Psi_n}_A}\). More precisely, let \(J\) be indices such that \(\{\ket{{\Psi_n}_A}\} = \{\ket{{\Psi_n}_A}:n \in J\}\) and for all \(n,m\in J\) with \(n\neq m\) we have \(\ket{{\Psi_n}_A} \neq \ket{{\Psi_m}_A}\). Let \(\tilde{J}_n = \{m : \ket{{\Psi_m}_A} = \ket{{\Psi_n}_A}\}\). Then we can write
\begin{equation}\label{eq.reddensdecomp}
\overline{\op{\rho}}(t) = \sum_{n\in J}  \overline{\sigma}_n \ket{{\Psi_n}_A}\bra{{\Psi_n}_A}
\end{equation}
where each term in the sum is orthogonal to the others and
\begin{equation}
\overline{\sigma}_n = \sum_{m \in \tilde{J}_n} \sigma_m \sum_k|\braket{{\Psi_m}_B}{\alpha_k}|^2.
\end{equation}

Suppose we have asymptotic expansions
\begin{align*}
\overline{\op{\rho}}(t) &= \overline{\op{\rho}}_0 + \lambda \overline{\op{\rho}}_1 + \lambda^2 \overline{\op{\rho}}_2 + \Od{\lambda^3} \\
\ket{{\Psi_n}_A} &= \ket{{\Psi_n^{(0)}}_A} + \lambda \ket{{\Psi_n^{(1)}}_A} + \lambda^2 \ket{{\Psi_n^{(2)}}_A} + \Od{\lambda^3} \\
\overline{\sigma_n} &= \overline{\sigma_n^{(0)}} + \lambda \overline{\sigma_n^{(1)}} + \lambda^2 \overline{\sigma_n^{(2)}} + \Od{\lambda^3}
\end{align*}
The above is indeed true if we take \(\lambda\) to be proportional to \(\op{H}_I(t)\) and take
\begin{align}
\label{eq.rho0full}\op{\rho}_0 &= \op{\rho}\left(t_0\right) \\
\label{eq.rho0red}\overline{\op{\rho}}_0 &= \overline{\op{\rho}}\left(t_0\right) =  \sum_k \left(\cdot\otimes\bra{\alpha_k}\right) \op{\rho}_0 \left(\cdot\otimes\ket{\alpha_k}\right) \\
\label{eq.rho1red}\overline{\op{\rho}}_1 &= -\frac{i}{\hbar} \int_{t_0}^t \sum_k \left(\cdot\otimes\bra{\alpha_k}\right) \left[\op{H}(t^\prime),\; \op{\rho}_0 \right]\left(\cdot\otimes\ket{\alpha_k}\right) \,dt^\prime\\
\label{eq.rho2red}\overline{\op{\rho}}_2 &= - \frac{1}{\hbar^2} \int_{t_0}^t \int_{t_0}^{t^\prime} \sum_k \left(\cdot\otimes\bra{\alpha_k}\right) \left\{ \op{H}_I (t^\prime) \op{H}_I(t^{\prime\prime}),\; \op{\rho}_0 \right\}\left(\cdot\otimes\ket{\alpha_k}\right)\,dt^{\prime\prime}\,dt^\prime \\
\nonumber&\hspace{2em}+ \frac{1}{\hbar^2} \int_{t_0}^t \int_{t_0}^{t} \sum_k \left(\cdot\otimes\bra{\alpha_k}\right) \op{H}_I (t^\prime) \,\op{\rho}_0\, \op{H}_I (t^{\prime\prime})\left(\cdot\otimes\ket{\alpha_k}\right)\,dt^{\prime\prime}\,dt^\prime.
\end{align}
as per equation (\ref{eq.rhoevol}).
...
\[
\overline{\sigma_n^{(1)}} = -\frac{i}{\hbar} \int_{t_0}^t \sum_k \left({\bra{{\Psi_n^{(0)}}_A}}\otimes\bra{\alpha_k}\right) \left[\op{H}(t^\prime),\; \op{\rho}_0 \right] \left({\ket{{\Psi_n^{(0)}}_A}}\otimes\ket{\alpha_k}\right)\,dt^\prime.
\]
Since \emph{[The line below needs to be corrected!]}
\[
\bra{\Psi_n^{(0)}} \op{H}(t^\prime) \op{\rho}_0 \ket{\Psi_n^{(0)}} = 
\sigma_n^{(0)} \bra{\Psi_n^{(0)}} \op{H}(t^\prime) \ket{\Psi_n^{(0)}} =
\bra{\Psi_n^{(0)}} \op{\rho}_0 \op{H}(t^\prime) \ket{\Psi_n^{(0)}}
\]
we have \(\overline{\sigma_n^{(1)}} = 0\).

\subsection{Von Neumann entropy}

The Von Neumann subsystem entropy for the state (\ref{eq.reddensdecomp}) is defined to be
\[
S = \sum_{n\in J} \overline{\sigma_n} \log \overline{\sigma_n}.
\]
Let \(I = \left\{n\in J : \overline{\sigma_n^{(0)}} \neq 0\right\}\). Using a derivation identical to the one in section \ref{sec.totalvonneumann}, we get that the change in subsystem entropy due to the interaction is
\begin{equation}\label{eq.subentropyresultwithrho2}
\boxed{\Delta S = -\lambda^2 \log\frac{1}{\lambda^2}\sum_{n \in I} \bra{{\Psi_n^{(0)}}_A}\overline{\op{\rho}_2}\ket{{\Psi_n^{(0)}}_A} + \Od{\lambda^2}.}
\end{equation}
This is as far as we can go for the non-separable state (\ref{eq.reddensdecomp}). For separable states, we can continue the derivation similarly to in section \ref{sec.totalvonneumann}, but these states remain separable and so are not that interesting.
