...
\label{eq.rho2red}\overline{\op{\rho}}_2 &= - \frac{1}{\hbar^2} \int_{t_0}^t \int_{t_0}^{t^\prime} \sum_m \left(\cdot\otimes\bra{\beta_m}\right) \left( \op{H}_I (t^\prime) \op{H}_I(t^{\prime\prime}) \;\op{\rho}(t_0) + \op{\rho}(t_0)\; \op{H}_I (t^{\prime\prime}) \op{H}_I(t^{\prime}) \right)\left(\cdot\otimes\ket{\beta_m}\right)\,dt^{\prime\prime}\,dt^\prime \\
...
We can now compute the change in entropy. We will consider two cases.

{\bf Case 1: All \(\overline{\sigma_n^{(0)}} \neq 0\).}
Then
\begin{align*}
...
{\bf Case 2: Some \(\overline{\sigma_n^{(0)}} = 0\).} This case is more interesting. We will consider it for the rest of our analysis.
...
\emph{[The above should be checked.]}
...
\subsection{Case of nicely diagonalized initial state}
