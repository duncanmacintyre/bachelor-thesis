...
\begin{equation}\label{eq.secondtdpe}
...
\end{equation}
In general, we can keep going to achieve higher order approximations. For us, however, the first and second order approximations are enough.
...
\section{Entropy analysis through perturbation theory}

Consider a Hermitian operator \(\op{A}_0\) with an eigenstate \(\ket{\psi_0}\) that has an eigenvalue \(\tau_0\). Consider another operator \(\op{A} = \op{A}_0 + \lambda \op{A}_1 + \Od{\lambda^2}\) with an eigenstate \(\ket{\psi} = \ket{\psi_0} + \lambda \ket{\psi_1} + \Od{\lambda^2}\) that has an eigenvalue \(\tau = \tau_0 + \lambda \tau_1 + \Od{\lambda^2}\). We have
\begin{align*}
(\op{A}_0 + \lambda \op{A}_1) \left(\ket{\psi_0} + \lambda \ket{\psi_1}\right) &= (\tau_0 + \lambda \tau_1) \left(\ket{\psi_0} + \lambda\ket{\psi_1}\right) + \Od{\lambda^2} \\
\op{A}_0 \ket{\psi_1} + \op{A}_1 \ket{\psi_0} &= \tau_0 \ket{\psi_1} + \tau_1 \ket{\psi_0} + \Od{\lambda} \\
\bra{\psi_0}\op{A}_0 \ket{\psi_1} + \bra{\psi_0}\op{A}_1 \ket{\psi_0} &= \tau_0\braket{\psi_0}{\psi_1} + \tau_1\braket{\psi_0}{\psi_0} + \Od{\lambda} \\
\tau_0\braket{\psi_0}{\psi_1} + \bra{\psi_0}\op{A}_1 \ket{\psi_0} &= \tau_0\braket{\psi_0}{\psi_1} + \tau_1 + \Od{\lambda} \\
\tau_1 &= \bra{\psi_0}\op{A}_1 \ket{\psi_0} + \Od{\lambda} \\
\tau_1 &= \bra{\psi_0}\op{A}_1 \ket{\psi_0}
\end{align*}
(since we take \(\tau_1\) not to depend on \(\lambda\)).

We will apply this result with \(\op{A} = \op{\rho}_\text{reduced}(t)\) and \(\op{A}_0 = \op{\rho}_\text{reduced}(t_0)\). In general, we can write \(\op{\rho}_\text{reduced}(t_0)\) in the form
\[
\op{\rho}_\text{reduced}(t_0) = \sum_i P_i \ket{\psi_i}\bra{\psi_i}.
\]
for some \(\ket{\psi_i}\) and \(P_i\) such that \(\sum_i P_i = 1\). It is clear that the eigenstates of \(\op{\rho}_\text{reduced}(t_0)\) are then the states \(\ket{\psi_i}\); these have eigenvalues \(P_i\). Take \(\widetilde{V}(p) = \lambda V^\star(p)\). Then we can read off the first order corrections for \(\op{\rho}\) from (\ref{eq.reddensevolution}).

\subsection{Pure initial state}

We will first consider the case where the initial state is pure, that is,
\[\op{\rho}_\text{reduced}(t_0) = \ket{\psi_0}\bra{\psi_0}\]
for some \(\ket{\psi_0}\). Then exactly one eigenvalue is non-zero; it is associated with the non-degenerate eigenstate \(\ket{\psi_0}\).

Let \(\{\epsilon_i\}\) be the eigenvalues of \(\op{\rho}_\text{reduced}(t)\) and choose them so that \(\epsilon_1 \approx 1\) and \(\epsilon_i \approx 0\) for \(i 
\geq 2\).

The von Neumann entropy is
\begin{align*}
S &= -\operatorname{tr}\left(\op{\rho}(t) \ln \op{\rho}(t)\right) \\
&= - \sum_{i=1}^\infty \epsilon_i \ln \epsilon_i \\
&= - \epsilon_1 \ln \epsilon_1 - \sum_{i=2}^\infty \epsilon_i \ln \epsilon_i.
\end{align*}
Now, \(x \ln x \approx x\) if \(x \approx 0\), so
\[
S 
\approx - \epsilon_1 \ln \epsilon_1 - \sum_{i=2}^\infty \epsilon_i.
\]
Also \(\epsilon_1 = 1 - \sum_{i=2}^\infty \epsilon_i\) so
\[
S 
\approx
- \epsilon_1 \ln \epsilon_1 + \epsilon_1 - 1.
\]
In particular, 
