...
Finally, we note that the key property here was that \(\braket{\text{photons} \sim}{\text{photons}}=0\). If instead \(\braket{\text{photons} \sim}{\text{photons}}\neq0\), decoherence will not occur, though the subsystem entropy will still increase in the same way. Thus the subsytem entropy quantifies decoherence only if we assume that \(\braket{\text{photons} \sim}{\text{photons}}=0\).
